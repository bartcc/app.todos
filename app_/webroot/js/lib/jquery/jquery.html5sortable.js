...
(function($){
  ...
  $.Html5Sortable.defaultOptions = {
    drag_target:        function( p_src ){
      return $( p_src );
    },
    text:               function( p_srcLine ){
      console.log(p_srcLine)
      return $('<div></div>').append( $( p_srcLine ).clone(true, true) ).html();
    },
    css:                'html5sortable-state-highlight',
    type:               $.Html5Sortable.DRAGANDDROP_DEFAULT_TYPE,
    drop:               function( p_srcLine, p_targetLine ) {
      return false;
    }
  };
  ...
  /**
     * Constructor
     */
  $.fn.Html5Sortable_ = function( p_options ) {
    ...
    return this.each(function(){
      ...
      var _initLi = function( $li ){
        ...
        _options.drag_target( $li ).attr('draggable', true).bind('dragstart', function(ev) {
          ...
          try {
            if (JSON.parse( ev.originalEvent.dataTransfer.getData("Text") ).type!= _options.type) {
              return true;
            }
          }catch(e){
            return true;
          ...
          try {
            if (ev.originalEvent.dataTransfer.getData("Text") && JSON.parse( ev.originalEvent.dataTransfer.getData("Text") ).type!= _options.type) {
              return true;
            }
          }catch(e){
            return true;
          ...
          var $item = $( JSON.parse( ev.originalEvent.dataTransfer.getData('Text') ).html ).hide();
          ...
          // data not saved
          if ( !_options.drop( $src.get(0), $item.get(0) ) ) {
            $item.remove();
            return false;
          }
      ...
      $this.bind('dragenter_', function(ev) {
        ...
        var $item = $( JSON.parse( ev.originalEvent.dataTransfer.getData('Text') ).html );
        $item.hide();
